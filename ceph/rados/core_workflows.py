...
class RadosOrchestrator:
    ...
    def change_osd_state(self, action: str, target: int, timeout: int = 180) -> bool:
        ...
        for _ in range(3):
            status_output, _, _, _ = host.exec_command(cmd=status_command, sudo=True, verbose=True, check_ec=True)
 
            if status_output.strip() == "activating" or status_output.strip() == "deactivating":
                log_info_msg = f"systemctl status is {status_output}" \
