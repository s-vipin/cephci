...
class RadosOrchestrator:
    ...
    def bench_write(self, pool_name: str, **kwargs) -> bool:
        ...
        byte_size = kwargs.get("byte_size", 4096).upper()
    ...
    def verify_pool_stats(self, pool_name, exp_objs: int, timeout=180) -> bool:
        ...
        while end_time > datetime.datetime.now():
            ...
            log.info(
                "| Objs in the pool post IOPS: {new_objs} " "| Expected",
                exp_objs,
                " or ",
                exp_objs + 1,
            )
            ...
            log.info(
                "Stats in the pool are yet to match expected object count ", exp_objs
            )
            log.info("Retrying check after 15 seconds")
            time.sleep(15)
        else:
            log.error(
                "Stats in the pool did not match the expected object count",
                exp_objs,
                " within timeout ",
                timeout,
            )
