...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        Cleanup Steps:
            ...
            4. Remove drain-related labels from host
        ...
        if "scenario1" in config.get("test_scenarios", []):
            ...
            service_id = (
                "osd_scenario_1" if deployment_type == "spec" else "osd.default"
            )
            new_osds = deploy_osd(
                rados_obj=rados_obj,
        ...
        if "scenario2" in config.get("test_scenarios", []):
            ...
            service_id = (
                "osd_scenario_2" if deployment_type == "spec" else "osd.default"
            )
            new_osds = deploy_osd(
                rados_obj=rados_obj,
                client_node=client_node,
        ...
        if "scenario3" in config.get("test_scenarios", []):
            ...
            service_id = (
                "osd_scenario_3" if deployment_type == "spec" else "osd.default"
            )
            new_osds = deploy_osd(
                rados_obj=rados_obj,
                client_node=client_node,
        ...
        if "scenario4" in config.get("test_scenarios", []):
            ...
            service_id = (
                "osd_scenario_4" if deployment_type == "spec" else "osd.default"
            )
            new_osds = deploy_osd(
                rados_obj=rados_obj,
                client_node=client_node,
        ...
        if "scenario5" in config.get("test_scenarios", []):
            ...
            service_id = (
                "osd_scenario_5" if deployment_type == "spec" else "osd.default"
            )
            new_osds = deploy_osd(
                rados_obj=rados_obj,
                client_node=client_node,
        ...
        if "scenario6" in config.get("test_scenarios", []):
            ...
            service_id = (
                "osd_scenario_6" if deployment_type == "spec" else "osd.default"
            )
            new_osds = deploy_osd(
                rados_obj=rados_obj,
                client_node=client_node,
