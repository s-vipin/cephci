...
def run(ceph_cluster, **kw):
    ...
    if config.get("stretch_mode"):
        ...
        try:
            ...
            log.info("Validating error generated when Mon is added to disallow list in classic mode")
            leader_mon = mon_obj.get_mon_quorum_leader()
            try:
                if mon_obj.set_disallow_mon(mon=leader_mon):
                    log.error(f"Mon {leader_mon} added to disallow list in classic Mode")
                    return 1
            except Exception as e:
                expected_err_msg = "Error EINVAL: You cannot disallow monitors in your current election mode"
                generated_err_msg = e.args[0].split("\n")[5]
                if (generated_err_msg != expected_err_msg):
                    log.error(
                        f"Expected error not generated when Mon added to disallow list in classic mode\n"
                        f"Expected error: {expected_err_msg}\n"
                        f"Generated error: {generated_err_msg}\n"
                        )
                    return 1
                log.info(
                    f"Expected error message generated {expected_err_msg}\n"
                    f"Successfully validated error generated when Mon is added to disallow list in classic mode"
                )
            
            check_rank_after_mon_removal(mon_obj=mon_obj, mon_workflow_obj=mon_workflow_obj, rados_obj=rados_obj)
            ...
            add_all_mons_to_disallow_list(mon_obj=mon_obj)

            disallowed_leader_checks(mon_obj=mon_obj, election_strategy="classic")
            ...
            disallowed_leader_checks(mon_obj=mon_obj, election_strategy="disallow")
...
def add_all_mons_to_disallow_list(mon_obj) -> None:
    ...
    mons = list(mon_obj.get_mon_quorum().keys())
    ...
    try:
        if mon_obj.set_disallow_mon(mon=last_allowed_mon):
            raise AssertionError(f"Last allowed mon {last_allowed_mon} added to disallow list") 
        ...
        raise AssertionError(ae.args[0])
            ...
            log.info(
                f"Validated Mon {last_allowed_mon} could not be added to diallow list\n"
                f"Error generated: {e.args}\n"
            )
...
def check_rank_after_mon_removal(mon_obj, mon_workflow_obj, rados_obj):
    ...
    def check_rank_change(quorum_before_mon_removal: dict, quorum_after_mon_removal: dict, removed_mon_rank: str) -> bool:
        ...
        Args:
            mons: Mon details before removal/Addition of Mon
        ...
        """
        log.info("Starting to validate Mon rank changes")
        for mon_name, mon_rank in quorum_before_mon_removal.items():
            if mon_rank == removed_mon_rank:
                quorum_before_mon_removal[mon_name] = len(quorum_before_mon_removal) - 1 
            elif mon_rank > removed_mon_rank:
                quorum_before_mon_removal[mon_name] = quorum_before_mon_removal[mon_name] - 1 

        for mon_name, mon_rank in quorum_after_mon_removal.items():
            if quorum_after_mon_removal[mon_name] != quorum_before_mon_removal[mon_name] :
                log.error("Ranks of Mons has not updated after Mon removal and addition")
                return False

        log.info("Completed validations for Mon rank change")
        return True

    log.info(
    "\n -------------------------------------------"
    "\n Starting validation:"
    "\n Rank updation on removing and adding Mon"
    "\n -------------------------------------------"
    )
        
    n = len(mon_obj.get_mon_quorum())
    ...
    for index in [ 0, n//2, -1]:
        ...
        quorum_before_mon_removal = mon_obj.get_mon_quorum()
        ...
        log.info(f"Validating rank updation when Mon name: {mon_name} rank: {mon_rank} is removed and added back")
        ...
        quorum_after_mon_removal = mon_obj.get_mon_quorum()

        log.info(f"Mon ranks before Mon removal: {quorum_before_mon_removal}")
        log.info("Proceeding to validate rank changes after Mon removal and addition")
        if not check_rank_change(quorum_before_mon_removal, quorum_after_mon_removal, mon_rank):
            raise AssertionError(
                f"Mon ranks are not updated"
            )
        log.info(f"Completed validation of rank changes when Mon with rank {mon_rank} is removed and added back")
...
def disallowed_leader_checks(mon_obj, election_strategy:str) -> bool:
    ...
    if original_election_strategy == 1 or original_election_strategy == election_strategy:
        log.error(
            "Cannot perform test in classic election strategy and"
            f" election strategy to switch cannot be same as current strategy"
            )
        return False
    ...
    lowest_ranked_mon = list(mon_obj.get_mon_quorum())[0]
    ...
    mon_obj.disallowed_leader_checks(lowest_ranked_mon)
    ...
    log.info(f"Proceeding to checks on lowest ranked Mon as disallowed leader")
    if election_strategy == "classic":
        ...
        if lowest_ranked_mon not in mon_obj.get_mon_quorum().keys():
    ...
    log.info(f"Successfully completed validation of lowest ranked Mon as disallowed leader")
    log.info("Proceeding to revert back cluster to previous state")
    log.info(f"Proceeding to revert election strategy to original state {original_election_strategy}")
    if not mon_obj.set_election_strategy(mode=original_election_strategy):
        raise AssertionError(f"could not set election strategy to {original_election_strategy} mode")

    log.info("Validate Mon is not leader, Mon is in quorum and Mon is in disallow leaders list "
            "after reverting election strategy to original state")
    mon_obj.disallowed_leader_checks(lowest_ranked_mon)

    log.info(f"Proceeding to revert disallowed leaders, removing Mon {lowest_ranked_mon} from list")
    if not mon_obj.remove_disallow_mon(mon=lowest_ranked_mon):
        raise AssertionError(f"could not remove Mon {lowest_ranked_mon} from disallowed list")
    
    log.info(
        f"Proceeding to perform post test scenario validations\n"
    ...
    if original_election_strategy == "disallow" and lowest_ranked_mon != current_leader:
        log.error("Mon with lowest rank is not leader after removal from disallowed leaders list")
        return False

    if lowest_ranked_mon not in mon_obj.get_mon_quorum().keys():
        log.error("Mon with lowest rank is not in Quorum after removal from disallowed leaders list")
        return False
    
    disallowed_list = mon_obj.get_disallowed_mon_list()
    if lowest_ranked_mon in disallowed_list:
        log.error("Mon with lowest rank is still in disalloweded leaders list post removal")
        return False
