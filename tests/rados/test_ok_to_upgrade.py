...
def run(ceph_cluster, **kw):
    ...
    ec_m = config.get("ec_m", 1)
    ceph_version = "20.2.1-120.el9cp"
    ...
    try:
        ...
        if "scenario1" in scenarios:
            ...
            expected = OsdOkToUpgradeCommandOutput(
                ...
                all_osds_upgraded=False,
                osds_in_crush_bucket=all_osds_host0,
                osds_ok_to_upgrade=sorted_osds[:2],
                osds_upgraded=[],
                bad_no_version=[],
            )
            assert expected == actual
...
def sort_osds_by_increasing_pg_count(
    ...
    sorted_osds = sorted(osds, key=lambda osd_id: pg_by_osd.get(int(osd_id), 0))
    log.info(
        "OSDs sorted by increasing PG count: %s (pg counts: %s)",
        sorted_osds,
        [pg_by_osd.get(int(osd_id), 0) for osd_id in sorted_osds],
    )
    return sorted_osds
