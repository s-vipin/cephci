...
def run(ceph_cluster, **kw):
    ...
    Scenarios:
        ...
        7. Revert to regular cluster when 1 mon daemon from DC1 is down
        8. Revert to regular cluster when 1 mon daemon from DC1 is down and 1
         mon daemon from DC2 is down
        9. Revert to regular cluster when tiebreaker mon is down
        10. Revert to regular cluster when all mons from DC1 is down
    """
    ...
    try:
        ...
        if "scenario10" in scenarios_to_run:
            log.info(test_seprator)
            revert_stretch_mode_scenarios.scenario10(default_crush_rule)
            log.info(test_seprator)
...
class RevertStretchModeScenarios(RevertStretchModeFunctionalities):
    ...
    def scenario1(self, crush_rule: dict, skip_wait_for_clean_pg_sets=False):
        """
    ...
    def scenario7(self, crush_rule):
        # Perform MON shutdown in stretch mode
        log.info("Scenario 7->  Shutdown 1 mon daemon in DC1 Host1")
        mon = self.site_1_mon_hosts[0]
    ...
    def scenario8(self, crush_rule):
        ...
        log.info(
            "Scenario 8 ->  Shutdown 1 mon daemon in DC1 Host1 and 1 mon daemon in DC2 Host1"
        )
    ...
    def scenario9(self, crush_rule):
        # Shut down the tiebreaker mon
        log.info("Scenario 9 ->  Shutdown tiebreaker mon daemon")
    ...
    def scenario10(self, crush_rule):
        # Shut down all mons in DC1
        log.info("Scenario 10 ->  Shutdown all mon daemons in DC1")
        ...
        # Revert stretch mode -> perform validation -> enable stretch mode
        self.scenario1(crush_rule, skip_wait_for_clean_pg_sets=True)
