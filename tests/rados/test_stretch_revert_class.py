...
class StretchMode:
    def __init__(self, **kwargs):
        self.pool_name = "revert_stretch_mode_pool" + generate_unique_id(4)
    ...
    def enable_stretch_mode(self, tiebreaker_mon):
        ...
        """
        stretch_enable_cmd = f"ceph mon enable_stretch_mode {tiebreaker_mon} stretch_rule datacenter"
    ...
    def create_pool_and_write_io(self, pool_name: str, client_node:CephNode):
        ...
        if self.pool_obj.do_rados_put(client=client_node, pool=pool_name, nobj=200, timeout=100) == 1:
...
class RevertStretchModeFunctionalities(StretchMode):
    ...
    def revert_stretch_mode(self, crush_rule=None):
        """
        ...
        """
        command = f"ceph mon disable_stretch_mode"
        if crush_rule is not None:
            command += f" {crush_rule['name']}"
        command += " --yes-i-really-mean-it"
        _ = self.rados_obj.run_ceph_command(command, print_output=True, client_exec=True)
    ...
    def validate_osd_configurations_post_revert(self, expected_osd_map_values):
        ...
        for config_name in osd_map_configs_to_validate:
            ...
            log_msg = (f" Expected {config_name} -> {expected_config_value}"
